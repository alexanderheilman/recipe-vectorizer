...
if __name__ == '__main__':
    if len(argv) != 3:
        print("Usage: python3 {} <max_count> <reverse?>".format(argv[0]))
        exit(1)
    ...
    while next_recipe:
        ...
        time.sleep(10)
        try:
            rating = get_recipe_info(browser)['rating_info']
            recipes_coll.update_one({'id':next_recipe['id']},
                                    {"$set":{'ratings_info': rating}}, upsert=False)
            ...
            print('Error reading rating: {}'.format(next_recipe['name']))
        next_recipe = get_next_recipe(results_coll, reverse=reverse)
    browser.quit()
